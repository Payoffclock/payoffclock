...
const Big = ({ label, value, sub, color = C.green, size = 34 }) => (
  ...
  <div style={{ fontSize: 12, color: C.muted, marginBottom: 5 }}>{label}</div>div>
  <div className="serif" style={{ fontSize: size, fontWeight: 400, color, lineHeight: 1.1, letterSpacing: "-0.5px" }}>{value}</div>div>
    {sub && <div style={{ fontSize: 12, color: C.faint, marginTop: 4 }}>{sub}</div>div>}
  </div>div>
  );
...
const Insight = ({ icon, text, hl, color = C.green }) => (
  ...
  <span style={{ fontSize: 18, flexShrink: 0, marginTop: 1 }}>{icon}</span>span>
  <div style={{ fontSize: 14, color: C.muted, lineHeight: 1.6 }}>
    {text}{hl && <span style={{ color, fontWeight: 600 }}> {hl}</span>span>}
  </div>div>
  </div>div>
  );
...
const AffLink = ({ href, children }) => (
  ...
  💡 <a href={href} target="_blank" rel="noopener noreferrer" style={{ color: C.teal }}>{children}</a>a>
  </div>div>
  );
...
const ArticleSection = ({ title, intro, faqs }) => (
  ...
  <h3 className="serif" style={{ fontSize: 24, marginBottom: 16, color: C.white }}>{title}</h3>h3>
    {intro.map((p, i) => (
    <p key={i} style={{ color: C.muted, fontSize: 15, lineHeight: 1.75, marginBottom: 14 }}>{p}</p>p>
    ))}
    {faqs && faqs.length > 0 && (
    <div style={{ marginTop: 24 }}>
    <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: C.green, marginBottom: 16 }}>Frequently Asked Questions</div>div>
      {faqs.map((f, i) => (
      <div key={i} style={{ marginBottom: 16 }}>
      <div style={{ fontWeight: 600, fontSize: 15, color: C.white, marginBottom: 5 }}>{f.q}</div>div>
      <div style={{ color: C.muted, fontSize: 14, lineHeight: 1.7 }}>{f.a}</div>div>
      </div>div>
      ))}
    </div>div>
  )}
...
const RangeRow = ({ label, min, max, step, value, onChange, prefix = "$", suffix = "" }) => (
  ...
  <span style={{ color: C.green, fontWeight: 700, fontSize: 16, minWidth: 80 }}>{prefix}{Number(value).toLocaleString()}{suffix}</span>span>
  </div>div>
  <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(Number(e.target.value))} />
  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.faint, marginTop: 3 }}>
  <span>{prefix}{Number(min).toLocaleString()}{suffix}</span>span>
  <span>{prefix}{Number(max).toLocaleString()}{suffix}</span>span>
  </div>div>
  </div>div>
  );
...
const NumInput = ({ label, value, onChange, step = 1, hint }) => (
  ...
  <input type="number" value={value} step={step} onChange={e => onChange(Number(e.target.value))} />
    {hint && <div style={{ fontSize: 11, color: C.faint, marginTop: 4 }}>{hint}</div>div>}
  </div>div>
  );
...
const CountdownRing = ({ pct, size = 120, label, value, color = C.green }) => {
  ...
  return (
    ...
    <text x="50" y="46" textAnchor="middle" fill={C.white} fontSize="13" fontWeight="700" fontFamily="Inter,sans-serif">{value}</text>text>
    <text x="50" y="60" textAnchor="middle" fill={C.muted} fontSize="8" fontFamily="Inter,sans-serif">{label}</text>text>
    </svg>svg>
    </div>div>
    );
};
...
const DebtFreeCountdown = () => {
  ...
  return (
    ...
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Debt-Free Countdown</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>Enter your debts. Watch the clock tick toward the day you owe nothing to anyone.</p>p>
    </div>div>
      ...
      <div><Label>Name</Label>Label><input type="text" value={d.name} onChange={e => upd(d.id, "name", e.target.value)} /></div>div>
      <div><Label>Balance ($)</Label>Label><input type="number" value={d.balance} onChange={e => upd(d.id, "balance", e.target.value)} /></div>div>
      <div><Label>Rate (%)</Label>Label><input type="number" value={d.rate} step="0.1" onChange={e => upd(d.id, "rate", e.target.value)} /></div>div>
      <div><Label>Min Payment ($)</Label>Label><input type="number" value={d.payment} onChange={e => upd(d.id, "payment", e.target.value)} /></div>div>
      <button onClick={() => rem(d.id)} style={{ background: "rgba(240,90,90,0.12)", border: "1px solid rgba(240,90,90,0.25)", color: C.red, borderRadius: 7, padding: "10px 12px", marginTop: 18, fontSize: 15 }}>✕</button>button>
      </div>div>
      </Card>Card>
      ))}
    <button onClick={addDebt} style={{ width: "100%", background: "transparent", border: `1px dashed ${C.greenDim}`, color: C.green, borderRadius: 10, padding: 12, fontSize: 14, marginBottom: 20 }}>+ Add a debt</button>button>
    ...
    <div style={{ fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: C.muted, marginBottom: 16 }}>Your debt-free date</div>div>
    ...
    <div style={{ color: C.muted, fontSize: 15, marginBottom: 28 }}>
      {yearsLeft > 0 && <span>{yearsLeft}yr </span>span>}
      {moLeft > 0 && <span>{moLeft}mo </span>span>}
      {dLeft > 0 && <span>{dLeft}d </span>span>}
    to go
    </div>div>
    ...
    <Card><Big label="Total Debt" value={fmt(totalOriginal)} color={C.red} /></Card>Card>
    <Card><Big label="Interest Saved with Extra" value={extra > 0 ? fmt(interestSaved) : "—"} color={C.green} sub={extra > 0 ? `${fmtMonths(monthsSaved)} sooner` : "Add extra payment"} /></Card>Card>
    <Card><Big label="Total Interest (With Strategy)" value={fmt(result.totalInt)} color={C.amber} /></Card>Card>
    </div>div>
    
      {result.perDebt.filter(d => d.monthPaidOff).length > 0 && (
      <Card style={{ marginBottom: 14 }}>
      <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 14, color: C.green }}>🏁 Payoff Milestones</div>div>
        ...
        return (
          ...
          <span style={{ color: C.white }}>{d.name}</span>span>
          <span style={{ color: C.green, fontWeight: 600 }}>{dt.toLocaleDateString("en-US", { month: "short", year: "numeric" })}</span>span>
          </div>div>
          );
      })}
      </Card>Card>
    ...
    <AffLink href="https://www.napfa.org/financial-planning/find-a-planner">Want a real plan? Find a fee-only financial advisor at NAPFA.org →</AffLink>AffLink>
...
const TriageCalc = () => {
  ...
  return (
    <div className="fade-up">
    <div style={{ marginBottom: 22 }}>
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Financial Triage</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>Every debt fighting for your money — here's who wins and who to attack first.</p>p>
    </div>div>
    ...
    <Card><Big label="Total Debt" value={fmt(totalDebt)} color={C.red} /></Card>Card>
    <Card><Big label="Monthly Minimums" value={fmt(totalMonthly)} color={C.white} /></Card>Card>
    <Card><Big label="Interest/Year If You Do Nothing" value={fmt(annualInterest)} color={C.red} sub="Your cost of inaction" /></Card>Card>
    </div>div>
    
    <Card>
    <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 16 }}>Attack Order</div>div>
      {sorted.map((d, i) => (
      ...
      <div style={{ width: 30, height: 30, borderRadius: "50%", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14, background: i === 0 ? C.green : C.surface2, color: i === 0 ? C.bg : C.white, boxShadow: i === 0 ? `0 0 10px ${C.greenDim}` : "none" }}>{i + 1}</div>div>
      <div style={{ flex: 1 }}>
      <div style={{ fontWeight: 500 }}>{d.name}</div>div>
      <div style={{ fontSize: 12, color: C.muted }}>{fmt(d.balance)} @ {d.rate}% APR</div>div>
      </div>div>
      <div style={{ textAlign: "right" }}>
      <div style={{ color: C.red, fontWeight: 600 }}>{fmt(d.balance * d.rate / 100 / 12)}/mo</div>div>
      <div style={{ fontSize: 11, color: C.faint }}>in interest</div>div>
      </div>div>
        {i === 0 && <div style={{ background: C.green, color: C.bg, borderRadius: 6, padding: "3px 9px", fontSize: 11, fontWeight: 700, whiteSpace: "nowrap" }}>ATTACK FIRST</div>div>}
      </div>div>
      ))}
      {extra > 0 && sorted[0] && (
    ...
    <AffLink href="https://www.napfa.org/financial-planning/find-a-planner">Want a pro to review your full picture? Find a fee-only advisor at NAPFA.org →</AffLink>AffLink>
...
const MortgageCalc = () => {
  ...
  return (
    <div className="fade-up">
    <div style={{ marginBottom: 22 }}>
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Mortgage Reality Check</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>The full picture on what that house actually costs — and how to own it years sooner.</p>p>
    </div>div>
    ...
    <div>
    <Label>Down Payment ({down}%)</Label>Label>
    <input type="range" min={3} max={50} step={1} value={down} onChange={e => setDown(Number(e.target.value))} style={{ marginTop: 8 }} />
    <div style={{ color: C.green, fontWeight: 700, marginTop: 4 }}>{down}% — {fmt(price * down / 100)} down</div>div>
    </div>div>
    <div>
    <Label>Interest Rate (%) — Today's avg: 6.56%</Label>Label>
    <input type="number" value={rate} step="0.01" onChange={e => setRate(Number(e.target.value))} />
    </div>div>
    <div>
    <Label>Loan Term</Label>Label>
    <select value={term} onChange={e => setTerm(Number(e.target.value))}>
    <option value={30}>30 Years</option>option>
    <option value={20}>20 Years</option>option>
    <option value={15}>15 Years</option>option>
    <option value={10}>10 Years</option>option>
    </select>select>
    </div>div>
      ...
      <div className="fade-up">
      <div className="g3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
      <Card><Big label="Principal & Interest" value={fmt(pmt)} sub="/month" /></Card>Card>
      <Card><Big label="Full PITI Payment" value={fmt(totalMonthly)} sub="w/ tax, insurance, PMI" color={C.teal} /></Card>Card>
      <Card><Big label="Affordability" value={statusLabel[status]} color={statusColor[status]} sub={`${Math.round(hRatio * 100)}% of gross income`} size={22} /></Card>Card>
      </div>div>
      ...
      <AffLink href="https://www.credible.com/mortgage">Compare mortgage rates from 20+ lenders at Credible →</AffLink>AffLink>
      </div>div>
    )}
      {tab === "payoff" && (
      <div className="fade-up">
      <div className="g3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
      <Card><Big label="Standard Payoff" value={fmtMonths(n)} sub={fmt(totalInt) + " total interest"} color={C.white} /></Card>Card>
      <Card><Big label="With Extra Payments" value={fmtMonths(withEx.months)} sub={extraPayment > 0 ? fmt(withEx.interest) + " total interest" : "Set extra above"} color={C.green} /></Card>Card>
      <Card><Big label="Interest Saved" value={extraPayment > 0 ? fmt(intSaved) : "—"} sub={moSaved > 0 ? fmtMonths(moSaved) + " sooner" : ""} color={C.lime} /></Card>Card>
      </div>div>
        {extraPayment > 0 && (
        <>
        ...
        <AffLink href="https://www.bankrate.com/mortgages/mortgage-rates/">Compare today's refinance rates at Bankrate →</AffLink>AffLink>
        </>>
        )}
      </div>div>
    )}
      ...
      <Card><Big label="Purchase Price" value={fmt(price)} color={C.white} /></Card>Card>
      <Card><Big label="Total Interest Over Loan Life" value={fmt(totalInt)} color={C.red} /></Card>Card>
      <Card><Big label="True Cost of This Home" value={fmt(trueCost)} color={C.red} sub="What you actually pay" /></Card>Card>
      <Card><Big label="Interest as % of Purchase Price" value={Math.round(totalInt / price * 100) + "%"} color={C.amber} sub="What the bank earns on you" /></Card>Card>
      </div>div>
      ...
      <AffLink href="https://www.credible.com/mortgage">See if a lower rate could save you thousands — compare at Credible →</AffLink>AffLink>
      </div>div>
    )}
...
const CostOfWaiting = () => {
  ...
  return (
    <div className="fade-up">
    <div style={{ marginBottom: 22 }}>
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Cost of Waiting</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>Press start and watch your interest accumulate in real time. Every second you wait costs you money.</p>p>
    </div>div>
    ...
    <div style={{ fontSize: 12, letterSpacing: "0.1em", textTransform: "uppercase", color: C.muted, marginBottom: 16 }}>Interest accumulating on your balance right now</div>div>
    ...
    <button onClick={reset} style={{ background: "transparent", border: `1px solid ${C.border}`, color: C.white, borderRadius: 10, padding: "13px 24px", fontSize: 16 }}>↺ Reset</button>button>
    </div>div>
    </Card>Card>
    
    <div className="g3" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 14 }}>
    <Card><Big label="Interest Per Day" value={"$" + daily.toFixed(2)} color={C.red} /></Card>Card>
    <Card><Big label="Interest Per Year (No Extra)" value={fmt(annual)} color={C.red} /></Card>Card>
    <Card><Big label="Total Interest (Min Payments)" value={moStd === Infinity ? "Never paid off" : fmt(totalInt)} color={C.red} sub={moStd !== Infinity ? fmtMonths(moStd) + " to pay off" : "Payment too low"} /></Card>Card>
    </div>div>
    ...
    <AffLink href="https://www.nerdwallet.com/best/credit-cards/balance-transfer">Stop the clock — compare 0% balance transfer cards at NerdWallet →</AffLink>AffLink>
...
const CreditScore = () => {
  ...
  const ScoreBar = ({ s, label, color }) => (
    ...
    <span style={{ color: C.muted }}>{label}</span>span>
    <span style={{ color, fontWeight: 700 }}>{Math.round(s)} — {band(s)[0]}</span>span>
    </div>div>
  ...
  return (
    <div className="fade-up">
    <div style={{ marginBottom: 22 }}>
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Credit Score Impact</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>See what's dragging your score down, what improving it saves you, and the exact moves to make.</p>p>
    </div>div>
    
    <div className="g2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
    <Card>
    <Label>Your Current Score (300–850)</Label>Label>
    <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 10 }}>
    <div className="serif" style={{ fontSize: 52, color: bColor, lineHeight: 1 }}>{score}</div>div>
    <div style={{ fontSize: 14, color: bColor, fontWeight: 600 }}>{bLabel}</div>div>
    </div>div>
    <input type="range" min={300} max={850} step={5} value={score} onChange={e => setScore(Number(e.target.value))} />
    </Card>Card>
    <Card>
    <div style={{ fontSize: 13, color: C.muted, marginBottom: 16 }}>What improving your score saves on a $300K mortgage:</div>div>
    ...
    <Card style={{ marginBottom: 20 }}>
    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 18, color: C.green }}>Score Factors — adjust to see impact</div>div>
    <div className="g2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
    <div>
    <Label>Credit Utilization: {util}%</Label>Label>
    ...
    <div>
    <Label>Late Payments (last 2 years)</Label>Label>
    <select value={lateCount} onChange={e => setLateCount(Number(e.target.value))}>
    <option value={0}>None — perfect payment history</option>option>
    <option value={1}>1 late payment</option>option>
    <option value={2}>2–3 late payments</option>option>
    <option value={4}>4+ late payments</option>option>
    </select>select>
    </div>div>
    <div>
    <Label>Average Account Age (years)</Label>Label>
    <input type="range" min={0} max={20} step={0.5} value={accountAge} onChange={e => setAccountAge(Number(e.target.value))} style={{ marginTop: 8 }} />
    <div style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>{accountAge} years average age</div>div>
    </div>div>
    <div>
    <Label>New Credit Inquiries (last 12mo)</Label>Label>
    <select value={newCredit} onChange={e => setNewCredit(Number(e.target.value))}>
    <option value={0}>0 — no new applications</option>option>
    <option value={1}>1 inquiry</option>option>
    <option value={2}>2–3 inquiries</option>option>
    <option value={5}>4+ inquiries</option>option>
    </select>select>
    </div>div>
    </div>div>
    </Card>Card>
    ...
    <Card style={{ marginBottom: 14 }}>
    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 18 }}>Your Score vs. Optimized</div>div>
    <ScoreBar s={score} label="Current score" color={bColor} />
    <ScoreBar s={optimized} label="Optimized score (30–90 days)" color={oColor} />
    <Hr />
    <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12, color: C.green }}>Your Action Plan</div>div>
    ...
    <AffLink href="https://www.experian.com/consumer-products/score-boost.html">Experian Boost can raise your score instantly using bills you already pay →</AffLink>AffLink>
...
const RentVsBuy = () => {
  ...
  return (
    <div className="fade-up">
    <div style={{ marginBottom: 22 }}>
    <h2 className="serif" style={{ fontSize: 28, marginBottom: 8 }}>Rent vs. Buy</h2>h2>
    <p style={{ color: C.muted, fontSize: 15, lineHeight: 1.6 }}>Honest math over your actual timeline — not a generic 30-year assumption.</p>p>
    </div>div>
    <Card style={{ marginBottom: 20 }}>
    <div className="g2" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
    ...
    <AffLink href="https://www.bankrate.com/mortgages/mortgage-rates/">Ready to buy? Compare today's mortgage rates at Bankrate →</AffLink>AffLink>
    
    <ArticleSection
      title="How This Comparison Is Calculated"
      intro={[
        "The rent-versus-buy decision isn't just 'mortgage payment vs. rent check' — it's the total cost of each path over the years you'd actually stay, accounting for the parts people tend to forget: closing costs, ongoing maintenance, home appreciation, and what you'd actually walk away with (or owe) if you sold at the end of that period.",
        "For buying, the calculator totals your down payment, closing costs (estimated at 3% of purchase price), every mortgage payment plus estimated maintenance over your timeframe, then nets that against your estimated home equity and sale proceeds after estimated selling costs. For renting, it totals your rent payments over the same period, increasing annually by your expected rent growth rate. Whichever number is lower is the better financial choice for that specific timeframe.",
        "Timeline is the single biggest lever in this decision. Closing costs and the upfront cost of a down payment need time to be offset by equity growth and appreciation — which is why buying tends to look worse over short stays (2–3 years) and progressively better the longer you plan to stay in the home.",
        "On a $320,000 home with $20,000 down at 6.56%, staying just 2–3 years often favors renting once closing costs are factored in — but stretch that to 7–10 years, and appreciation plus accumulated equity typically tips the math toward buying, even against a comparable rent.",
        ]}
      faqs={[
        { q: "Does this include what I'd earn by investing my down payment instead of buying?", a: "Not directly — the calculator focuses on housing costs specifically. If you're weighing whether to invest your down payment in the market instead of a home, that's a separate opportunity-cost comparison worth running alongside this one, since market returns and home appreciation don't move in lockstep." },
        { q: "How sensitive is the result to the home appreciation assumption?", a: "Very. Appreciation compounds over your timeline, so even a 1–2 percentage point change in the assumed annual rate can shift the outcome, especially over longer stays. Try adjusting the appreciation slider up and down to see how much your specific result depends on that assumption before treating either number as certain." },
        { q: "What's included in the 3% closing cost estimate?", a: "It's a rough placeholder covering typical buyer closing costs — loan origination fees, appraisal, title insurance, and similar one-time costs — which commonly fall in the 2–5% range of the purchase price depending on your location and lender. Get an actual estimate from your lender for a precise number." },
        { q: "If I'm planning to stay long-term, should I always buy?", a: "Long timelines generally favor buying in this model because equity and appreciation have more time to outweigh the upfront costs, but it's not automatic — a home with low or flat appreciation in your area, high property taxes, or high maintenance costs can still favor renting even over a decade. Run your specific numbers rather than assuming the general trend applies to your market." },
        ]}
      />
    </div>div>
    );
...
const EmailCapture = () => {
  ...
  return (
    <Card style={{ background: `linear-gradient(135deg, ${C.surface} 0%, rgba(74,200,120,0.08) 100%)`, marginTop: 36 }}>
      {!sent ? (
      <>
      <div className="serif" style={{ fontSize: 22, marginBottom: 8 }}>Get your debt-free date in your inbox</div>div>
      <p style={{ fontSize: 14, color: C.muted, marginBottom: 18, lineHeight: 1.6 }}>Enter your email and we'll send you a monthly progress update — your savings, your milestones, your countdown. Free, forever.</p>p>
      ...
      <button onClick={() => email.includes("@") && setSent(true)} style={{ background: C.green, color: C.bg, border: "none", borderRadius: 8, padding: "10px 24px", fontWeight: 700, fontSize: 15 }}>Get My Date →</button>button>
      </div>div>
      <div style={{ fontSize: 11, color: C.faint, marginTop: 10 }}>No spam. Unsubscribe anytime. Your data stays yours.</div>div>
      </>>
      ) : (
      <div style={{ textAlign: "center", padding: "16px 0" }}>
      <div style={{ fontSize: 32, marginBottom: 10 }}>✅</div>div>
      <div className="serif" style={{ fontSize: 22, color: C.green, marginBottom: 6 }}>You're on the list.</div>div>
      <div style={{ color: C.muted, fontSize: 14 }}>Your first debt-free update is on its way.</div>div>
      </div>div>
    )}
    </Card>Card>
    );
...
export default function App() {
  ...
  return (
    ...
    The money math<br />
    <span style={{ color: C.green }}>the banks don't show you.</span>span>
    </h1>h1>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 30 }}>
      ...
      <div style={{ fontSize: 20, marginBottom: 7 }}>{t.icon}</div>div>
      <div style={{ fontWeight: 600, fontSize: 13, lineHeight: 1.3, marginBottom: 3 }}>{t.label}</div>div>
      <div style={{ fontSize: 11, opacity: 0.65 }}>{t.sub}</div>div>
      </button>button>
      ))}
    </div>div>
    ...
    <a href="/about.html" style={{ color: C.green, margin: "0 10px", textDecoration: "none" }}>About</a>a>
    <a href="/contact.html" style={{ color: C.green, margin: "0 10px", textDecoration: "none" }}>Contact</a>a>
    <a href="/privacy.html" style={{ color: C.green, margin: "0 10px", textDecoration: "none" }}>Privacy Policy</a>a>
    <a href="/terms.html" style={{ color: C.green, margin: "0 10px", textDecoration: "none" }}>Terms of Use</a>a>
    </div>div>
